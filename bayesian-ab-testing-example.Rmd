...
output$plot_distribution <- renderPlotly({
  f_plot_distributions(df_summary %>% filter(test_name == input$selected_test), 
...
output$plot_daily_rates <- renderPlotly({
  f_plot_daily_rate(df_daily %>% filter(test_name == input$selected_test),input$alpha_select, input$beta_select)
})
...
test_results <- reactive({
    ...
    results <- df %>%
      group_by(variation) %>%
      mutate(results = list(f_test_variations(success_a = successes,
                                              failure_a = failure,
                                              input$alpha_select,
                                              input$beta_select,
                                              control_samples,
    ...
    results <- results %>%
      ...
      mutate(rate = scales::percent(rate),
