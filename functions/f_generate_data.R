...
f_generate_data <- function() {
  ...
  variations <- tibble(
    variation = c('Variation A','Variation B','Control')
  )
  
  df_params <- crossing(tests, variations) %>%
    group_by(test_name) %>%
    mutate(
      trials_param = round(rnorm(1,mean=200, sd = 10))
    ) %>% 
    group_by(test_name, variation) %>%
    mutate(
      success_param = round(rnorm(1,mean=40, sd = 4))
    ) %>% 
    ungroup()
  
  df_daily <- df_params %>%
    mutate(date = map2(start_date, end_date, seq, 'day')) %>%
    unnest()
